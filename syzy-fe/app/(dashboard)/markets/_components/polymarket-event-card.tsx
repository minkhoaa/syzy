...
import { ExternalLink, Plus, TrendingUp, Layers } from "lucide-react";
import { Card } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";
import type { PolymarketEvent } from "@/features/markets/hooks/use-polymarket-events";
...
export function PolymarketEventCard({ event }: PolymarketEventCardProps) {
  ...
  const isBinary =
    ...
    event.markets[0].outcomes[0]?.toLowerCase() === "yes";
  ...
  return (
    ...
    <Card className="group relative flex flex-col bg-card/60 backdrop-blur-md hover:bg-card/80 border border-white/5 hover:border-primary/20 h-[280px] overflow-hidden rounded-3xl transition-all duration-500 shadow-sm hover:shadow-xl">
      ...
      <div className="relative z-10 px-5 pt-5 pb-2 flex gap-3 shrink-0">
        ...
        <div className="min-w-0 flex-1 flex flex-col justify-start gap-1.5">
          <div className="flex flex-wrap items-center gap-1.5 overflow-hidden">
            <Badge
              variant="secondary"
              className="px-1.5 py-0 h-4 text-[9px] font-bold tracking-wider uppercase border-0 bg-blue-500/10 text-blue-500 rounded-sm"
            >
              Polymarket
            </Badge>
            {!isBinary && (
      ...
      <div className="relative z-10 px-5 pb-4 pt-3 mt-auto shrink-0 flex items-center justify-between border-t border-border/40 bg-background/20">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-baseline gap-1 font-mono cursor-default" title="Liquidity">
            <span className="font-bold tracking-tight text-[13px] text-foreground/80">
              {formatCompact(event.liquidity)}
            </span>
            <span className="text-[10px] text-muted-foreground/60 uppercase tracking-widest font-sans">Liq</span>
          </div>
        </div>
    ...
    <CloneMarketDialog
      ...
      polymarketMeta={{
        conditionId: isBinary ? event.markets[0].conditionId : undefined,
        slug: event.slug,
        marketId: isBinary ? event.markets[0].id : undefined,
        resolutionSource: isBinary
          ? event.markets[0].resolutionSource || "Polymarket UMA Oracle"
